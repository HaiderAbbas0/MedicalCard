...
class LabService {
  ...
  Future<List<LabOrderModel>> queue() async {
    ...
    final list =
        rows
            .where(
              (r) =>
                  !['released_to_patient', 'cancelled'].contains(r['status']),
            )
            .map((r) {
              final m = Map<String, dynamic>.from(r);
              m['patient'] = {
                'display_name': _mask(
                  (r['patient'] as Map?)?['full_name']?.toString() ?? '',
                ),
              };
              return m;
            })
            .toList()
          ..sort((a, b) {
            final pr = (_rank[a['priority']] ?? 9).compareTo(
              _rank[b['priority']] ?? 9,
            );
            return pr != 0
                ? pr
                : (a['ordered_at'] ?? '').toString().compareTo(
                    (b['ordered_at'] ?? '').toString(),
                  );
          });
  ...
  Future<void> uploadResult(
    ...
    await db.from('lab_results').insert({
      ...
      if (filePath != null) 'result_file_path': filePath,
    ...
    await db
        ...
        .update({
          'status': 'resulted',
          'resulted_at': DateTime.now().toIso8601String(),
        })
        .eq('id', orderId);
  ...
  Future<void> uploadResultFile(
    ...
    final order = await db
        ...
        .single();
    final objectPath =
        '${order['patient_id']}/$orderId/${DateTime.now().millisecondsSinceEpoch}_$name';
