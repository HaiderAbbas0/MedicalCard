  ...
  Widget build(BuildContext context) {
    ...
    return _SettingsScaffold(
      ...
      children: [
        ...
        GradientButton(
          label: _busy ? 'Saving…' : 'Update password',
...
class LoginSecurityScreen extends StatelessWidget {
    ...
    return _SettingsScaffold(
      ...
      children: [
        _card(
          context,
          children: [
            _row(context, 'Hayaat ID', user?.cardNumber ?? '—', mono: true),
  ...
  Widget _row(
    ...
    return Row(
      children: [
        Expanded(
          child: Text(label, style: AppText.body.copyWith(color: c.text2)),
        ),
        Flexible(
          child: Text(
            value,
            textAlign: TextAlign.right,
            style: (mono ? AppText.mono : AppText.bodyStrong).copyWith(
              fontSize: 14,
...
class _ConsentManagementScreenState extends State<ConsentManagementScreen> {
  static const _items = [
    (
      'share_history',
      'Share full history with treating doctors',
      'Doctors you visit can see your past visits, prescriptions and lab results.',
      true,
    ),
    (
      'emergency_access',
      'Emergency access',
      'Allow verified emergency staff to view critical info (blood group, allergies) via your card.',
      true,
    ),
    (
      'research',
      'Anonymous research',
      'Allow your de-identified data to support public-health research.',
      false,
    ),
    (
      'notifications',
      'Health notifications',
      'Receive reminders and updates about your care.',
      true,
    ),
  ];
  ...
  Future<void> _load() async {
    ...
    try {
      server = await ComplianceService().consentPreferences();
    } catch (_) {
      server = {};
    }
    setState(() {
      for (final i in _items) {
        _state[i.$1] = server[i.$1] ?? prefs.getBool('consent_${i.$1}') ?? i.$4;
      }
      _loading = false;
    });
  }
  ...
  Future<void> _set(String key, bool value) async {
    ...
    try {
      await ComplianceService().setConsentPreference(key, value);
    } catch (e) {
      if (!mounted) return;
      setState(() => _state[key] = !value);
      await prefs.setBool('consent_$key', !value);
  ...
  Widget build(BuildContext context) {
    ...
    return _SettingsScaffold(
      ...
      children: [
        Text(
          'Control how your health information is shared. You can change these at any time.',
          style: AppText.body.copyWith(color: c.text2),
        ),
        const SizedBox(height: 18),
        if (_loading)
  ...
  Future<void> _requestDelete() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Deletion request submitted. You will be signed out.'),
        ),
      );
  ...
  Widget build(BuildContext context) {
    ...
    return _SettingsScaffold(
      ...
      children: [
        ...
        const SizedBox(height: 10),
        Text(
          'Version ${LegalText.version} · Last updated ${LegalText.lastUpdated}',
...
class HelpSupportScreen extends StatelessWidget {
  ...
  static const _faqs = [
    (
      ...
      'Open "My Card" from the dashboard and tap "Request your card". Fill in your details and photo, and a digital card is issued instantly with your 16-digit Hayaat ID.',
    ),
    (
      'How do medicine reminders work?',
      'When a doctor prescribes a medicine with a daily schedule, it appears under Prescriptions → Reminders. Mark each dose as Taken or Skipped to track your adherence.',
    ),
    (
      'Who can see my medical records?',
      'Only you and the healthcare staff you visit. You can fine-tune sharing under Settings → Consent management.',
    ),
    (
      'How do I order a physical card?',
      'From your card screen, choose "Order physical card". It is free — you only pay a small cash-on-delivery charge to the courier.',
    ),
  ];
        ...
        children: [
          ...
          AnimatedCrossFade(
            ...
            secondChild: Padding(
              ...
              child: Text(
                widget.answer,
                style: AppText.caption.copyWith(color: c.text2, height: 1.5),
              ),
            ),
