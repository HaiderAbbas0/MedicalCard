...
class _NewEncounterScreenState extends State<NewEncounterScreen> {
  ...
  String _specialty = MedicalSpecialties.general.name;
  static final _specialties = MedicalSpecialties.all
  ...
  Future<void> _addMedication() async {
              ...
              children: [
                TextField(
                  ...
                  decoration: const InputDecoration(
                    hintText: 'Medication name (e.g. Amoxicillin)',
                  ),
                ),
                ...
                TextField(
                  ...
                  decoration: const InputDecoration(
                    hintText: 'Duration in days (e.g. 7)',
                  ),
                ),
                ...
                CheckboxListTile(
                  ...
                  onChanged: (v) => setLocal(() => morning = v ?? false),
                ),
                CheckboxListTile(
                  dense: true,
                  contentPadding: EdgeInsets.zero,
                  ...
                  onChanged: (v) => setLocal(() => afternoon = v ?? false),
                ),
                CheckboxListTile(
                  dense: true,
                  contentPadding: EdgeInsets.zero,
                  ...
                  onChanged: (v) => setLocal(() => evening = v ?? false),
                ),
                CheckboxListTile(
                  dense: true,
                  contentPadding: EdgeInsets.zero,
                  ...
                  onChanged: (v) => setLocal(() => night = v ?? false),
                ),
              ],
            ),
          ),
    ...
    await _run(() async {
      final res = await widget.service.addMedication(
        _encounterId!,
        name,
        dosageUnit: strengthCtrl.text.trim().isEmpty
            ? null
      ...
      final label =
          '$name${strengthCtrl.text.trim().isNotEmpty ? ' ${strengthCtrl.text.trim()}' : ''}'
  ...
  Future<void> _addVital() async {
    final value = await _prompt(
      'Record vital',
      'e.g. Blood Pressure Systolic = 130',
    );
    if (value == null || value.isEmpty || _encounterId == null) return;
    await _run(() async {
  ...
  Future<void> _addLabOrder() async {
    ...
    if (_labs.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('No active labs available.')),
      );
      return;
    }
    ...
    if (ok == true && testCtrl.text.trim().isNotEmpty) {
      await _run(() async {
        await widget.service.addLabOrder(
          _encounterId!,
          testCtrl.text.trim(),
          labId,
          priority: priority,
        ...
        setState(
          () => _labOrders.add(
            '${testCtrl.text.trim()} (${priority.toUpperCase()})',
          ),
        );
      });
    }
  ...
  Future<void> _finalize() async {
    ...
    try {
      ...
      if (_chiefCtrl.text.trim().isNotEmpty)
        patch['chief_complaint'] = _chiefCtrl.text.trim();
      if (_followUpCtrl.text.trim().isNotEmpty) {
        patch['follow_up_required'] = true;
        patch['follow_up_date'] = _followUpCtrl.text.trim();
      }
      if (patch.isNotEmpty)
        await widget.service.updateEncounter(_encounterId!, patch);
      ...
      body: _creating
          ...
          : ListView(
              ...
              children: [
                ...
                Text(
                  'Hayaat ID ${formatHayaatId(widget.patient.cardNumber)}',
              ...
              (e) => Padding(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(e, style: TextStyle(color: c.text2)),
                    ),
                  ],
                ),
              ),
