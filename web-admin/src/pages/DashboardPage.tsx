...
const CARDS: { key: keyof DashboardStats; label: string; icon: string; tint: string }[] = [
  { key: 'total_patients', label: 'Registered Patients', icon: 'patients', tint: 'var(--accent)' },
  ...
  { key: 'appointments_today', label: 'Appointments Today', icon: 'calendar', tint: 'var(--accent)' },
  { key: 'pending_lab_orders', label: 'Pending Lab Orders', icon: 'pill', tint: 'var(--blue)' },
  { key: 'total_clinics', label: 'Clinics', icon: 'clinic', tint: 'var(--accent)' },
...
export default function DashboardPage() {
  ...
  if (!stats) return <StatGridSkeleton count={8} />;
